...
exports.createPayment = async (req, res) => {
  ...
  try {
    ...
    if (paymentIntent.status === 'requires_confirmation') {
      ...
      if (confirmedPaymentIntent.status === 'requires_action' || confirmedPaymentIntent.status === 'requires_source_action') {
        ...
        return res.status(400).json({
          success: false,
          mes: "Thanh toán không thành công. Chi tiết: " + (confirmedPaymentIntent.charges.data[0]?.failure_message || 'Vui lòng kiểm tra thông tin.'),
        });
      } else {
        return res.status(400).json({
          success: false,
          mes: "Trạng thái thanh toán không xác định: " + confirmedPaymentIntent.status,
        });
      }
      ...
      return res.status(400).json({
        success: false,
        mes: "Thanh toán không thành công. Chi tiết: " + (paymentIntent.charges.data[0]?.failure_message || 'Vui lòng kiểm tra thông tin.'),
      });
    } else {
      return res.status(400).json({
        success: false,
