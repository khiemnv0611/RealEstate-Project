    ...
    try {
      ...
      for (const user of users) {
        ...
        const expirationDate = new Date(
          planRegisterDate.getTime() +
            user.membershipPlan.duration * 24 * 60 * 60 * 1000
        );
        ...
        if (now > expirationDate) {
          ...
          console.log(
            `User ${user.id}'s membership plan has expired. Reverted to default.`
          );
        }
      }
      ...
      for (const property of properties) {
        ...
        const expirationDate = new Date(
          updatedAt.getTime() +
            property.rOwner.membershipPlan.postDate * 24 * 60 * 60 * 1000
        );
        ...
        if (now > expirationDate) {
          ...
          console.log(
            `Property ${property.id}'s membership plan has expired. Reverted to default.`
          );
        }
      }
    } catch (error) {
