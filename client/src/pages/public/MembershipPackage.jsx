...
const MembershipPackage = () => {
  ...
  const handleRegister = async (id, name, price) => {
    Swal.fire({
      icon: "warning",
      title: "Xác nhận đăng ký gói " + name + " với giá " + price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
      ...
      if (result.isConfirmed) {
        ...
        if (res.success) {
          Swal.fire("Thành công!", "", "success").then(() => {
            ...
            navigate(`${path.OWNER_LAYOUT}/${path.PERSONAL}`)
          });
        }
        else {
      ...
      <div className="flex items-center justify-center w-full relative gap-6">
        {plans?.map((plan) => (plan.id != 1 && (
          <div className="min-w-[460px] flex flex-col gap-6 rounded-md -mt-10 bg-white shadow-lg p-6 transition-transform border border-transparent transform hover:-translate-y-4 hover:shadow-2xl hover:border-blue-700 h-96">
            ...
            {plan.id != 1 && (
              <div className="px-4 py-3 w-full font-bold text-center border border-main-800 text-main-700 rounded-md hover:bg-main-300" onClick={() => handleRegister(plan.id, plan.name, plan.price)}>
                Mua ngay
              </div>
